"""Run the deterministic Builder source-generation milestone."""
...
MILESTONE_ID = "flow_generated"
OUTPUT_IDS = ("workflow_source_bundle", "staged_harness")
...
def _load_builder_steps() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("m8m_generate_from_audit_steps", path)
...
def _context(
    ...
    if resolved_run_dir is None:
        raise ValueError("generate_from_audit requires run_dir")
    ...
    if declared_step and declared_step != MILESTONE_ID:
        raise ValueError(f"generate_from_audit cannot run milestone {declared_step}")
...
def run(
    ...
    **_: Any,
    ...
    if draft is not None:
        raise ValueError("generate_from_audit does not accept drafts")
    ...
    if not isinstance(outputs, dict) or set(outputs) != set(OUTPUT_IDS):
        raise RuntimeError("generate_from_audit handler returned unexpected output ports")
    return result
