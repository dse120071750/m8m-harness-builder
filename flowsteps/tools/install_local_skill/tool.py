"""Run the deterministic Builder local-installation milestone."""
...
MILESTONE_ID = "skill_shipped"
OUTPUT_IDS = ("installation_receipt",)
...
def _load_builder_steps() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("m8m_install_local_skill_steps", path)
...
def _context(
    ...
    if resolved_run_dir is None:
        raise ValueError("install_local_skill requires run_dir")
    ...
    if declared_step and declared_step != MILESTONE_ID:
        raise ValueError(f"install_local_skill cannot run milestone {declared_step}")
...
def run(
    ...
    **_: Any,
    ...
    if draft is not None:
        raise ValueError("install_local_skill does not accept drafts")
    ...
    if not isinstance(outputs, dict) or set(outputs) != set(OUTPUT_IDS):
        raise RuntimeError("install_local_skill handler returned unexpected output ports")
    return result
