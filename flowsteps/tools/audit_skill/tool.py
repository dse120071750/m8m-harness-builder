"""Run the deterministic Builder source-audit milestone."""
...
MILESTONE_ID = "audit_complete"
OUTPUT_IDS = ("source_audit",)
...
def _load_builder_steps() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("m8m_audit_skill_steps", path)
...
def _context(
    ...
    if resolved_run_dir is None:
        raise ValueError("audit_skill requires run_dir")
    ...
    if declared_step and declared_step != MILESTONE_ID:
        raise ValueError(f"audit_skill cannot run milestone {declared_step}")
...
def run(
    ...
    **_: Any,
    ...
    if draft is not None:
        raise ValueError("audit_skill does not accept drafts")
    ...
    if not isinstance(outputs, dict) or set(outputs) != set(OUTPUT_IDS):
        raise RuntimeError("audit_skill handler returned unexpected output ports")
    return result
