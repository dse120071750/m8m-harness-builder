"""Run the deterministic Builder harness-validation milestone."""
...
MILESTONE_ID = "harness_validated"
OUTPUT_IDS = ("validation_report",)
...
def _load_builder_steps() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("m8m_validate_harness_steps", path)
...
def _context(
    ...
    if resolved_run_dir is None:
        raise ValueError("validate_harness requires run_dir")
    ...
    if declared_step and declared_step != MILESTONE_ID:
        raise ValueError(f"validate_harness cannot run milestone {declared_step}")
...
def run(
    ...
    **_: Any,
    ...
    if draft is not None:
        raise ValueError("validate_harness does not accept drafts")
    ...
    if not isinstance(outputs, dict) or set(outputs) != set(OUTPUT_IDS):
        raise RuntimeError("validate_harness handler returned unexpected output ports")
    return result
