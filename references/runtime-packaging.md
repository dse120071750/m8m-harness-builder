...
This reference applies only to `--mode package`. Ordinary workflow creation and
edits use `builder-authoring.md`; they do not inherit the release, migration,
closed in-process tool, JPEG, or installation requirements below.
...
Local installation also admits these exact declared files outside the standard
managed directories. It overlays their containing directories atomically,
preserving unrelated files. Nested overlays share only a containing directory
already required by another declared file; installation never invents a broader
common root. A dependency at the repository top level, or one whose directory
would overlap a generated managed root, requires a different source layout and
is rejected. The installer does not swap the whole repository. Declared source
and destination paths must match, and runtime-isolation checks still apply.

When project Python uses package imports, declare `codebase_import_roots` on the
canvas as an ordered list of at most sixteen exact codebase-relative directories;
use `.` explicitly for the codebase root. The compiler preserves that order in
canonical flow bytes and implementation identity. Every root must be covered by
declared implementation Python files. The runtime derives an exact module and
namespace-parent allowlist from those files, without adding directories to
`sys.path`. Uncaptured siblings, ambiguous names, unsafe paths, foreign cached
modules, and names owned by the standard library or verified vendor packages
fail closed. The scope covers handler import, execution, lazy imports and
separate cycle/judge tools. Omission retains historical import behavior; adding
the declaration requires a newly packaged immutable runtime release.

Declare additional third-party product dependencies in `runtime_distributions`
as an ordered list of at most 32 closed `{name, version}` objects, for example
`[{name: Pillow, version: "12.1.0"}, {name: requests, version: "2.32.4"}]`.
Names must be unique after distribution-name normalization. Versions are exact;
ranges, extras, direct URLs and installation commands are unsupported. The Builder
uses only installed metadata to close these roots together with its core runtime
dependencies over active host markers. Active transitive extras and URLs reject
explicitly. Every resolved exact version, native extension, bundled DLL and data
file is included in the immutable dependency/file identity. The Python ABI and
executable digest bind this platform-specific release. The generated isolated
launcher disables system site-packages, and startup checks the authored pins
against the verified runtime lock. Omission retains the existing core roots.
On Windows the verified vendor roots use extended-length absolute import paths
so native extension loaders can open digest-addressed paths beyond MAX_PATH.
This changes path transport only; it adds no new dependency search locations.
...
Each Gem is its milestone's complete master prompt. Author it before binding
FlowSteps; the whole prompt leads execution. `success` and the output declarations
remain the machine completion contract. During migration, a legacy `## Rule of success`
section is optional, but when present its normalized text must exactly match
`success`. Before its first FlowStep, a candidate worker receives the complete
master prompt, bound inputs, and derived expectation. It returns only `{outputs: {...}}`; candidate or draft `ok` values
...
Milestone input, draft, tool, and output schemas may use ordinary file paths
without hash companions. Runtime admission checks actual named outputs and
copies file/media outputs into the run. Do not add revisions, proof graphs,
or automatic image reviews to active milestone paths. Explicit package and
external API integrity contracts may retain their own checksums; these do not
create general milestone requirements. A separate semantic judge is opt-in.
...
Each milestone owns a complete master prompt at `references/<id>.md`.
Optional `## <flowstep>` sections add implementation notes; they never replace
the full prompt. The judge receives the
...
| `<repo>/flowsteps/flows/<id>/references/<id>.md` | Complete milestone master prompt |
