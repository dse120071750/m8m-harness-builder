...
flowchart TD
    ...
    plan_frozen["plan_frozen<br/>intel:completion<br/>asset:file<br/>hash_bind,schema_validate"]
...
| `plan_frozen` | `file` | `completion` | `hash_bind`, `schema_validate` | linear |
...
None. Linear chain; no `next.when`.

## Loops (foreach)

None. No typed array with `maxItems` on a tool milestone.
