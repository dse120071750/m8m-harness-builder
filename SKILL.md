...
- **wait:** not a fourth canvas word. It is a **milestone**
  (`response_ready`) with N FlowSteps + one judge. Pause = no draft
  yet (`ACTION_REQUIRED`). Resume = write `work/draft.json` and
  `advance()`. Judge reads this gem: pass receipt → next; not ok →
  keep working; no asset → BLOCK. Do not add `loop: wait`.
- **branch:** after this milestone’s asset PASSes, AI drafts which
  path to take. The worker writes `{ok, branch}`. Skip is not BLOCK.
  Not a judge module. Path because of what they said is branch
  **after** Response is ready, not wait-as-IF.
...
- id: response_ready
  ...
  success: "Response is ready — must produce a json proof."
  flowsteps:
    - { id: ask_written, tool: ask_write }
    - { id: response_bound, tool: slot_write }
```
...
<run>/
  ...
  milestones/response_ready/work/ask.json     # frozen question
  milestones/response_ready/work/draft.json   # incoming reply (resume)
```
